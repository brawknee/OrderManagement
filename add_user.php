...
<?php					
	if(isset($_POST['submit'])){
		...
		if (isset($error)) {
			header('Location: add_user.php?e='.urlencode($error)); exit;
		}
		...
		if($num > 0){	
            header('Location: add_user.php?e='.urlencode("Username Already Exist")); exit;
		}
        
        $query=mysqli_query($connection, "INSERT INTO user_tbl(`fullname`,`phone`,`email`,`username`,`password`) VALUES ('$fullname','$phone','$email','$username','$password')") or die (mysqli_error($connection));
        if($query){
            header('Location: add_user.php?s='.urlencode("<b>User Successfully Registered.</b>")); 		
            exit;
		}
}
?>	       
