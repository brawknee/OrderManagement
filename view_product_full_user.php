...
if(isset($_POST['add_to_cart'])){
    if(!isset($_SESSION['registration_id'])){
        ...
        $quer=mysqli_query($connection, "INSERT INTO cart_tbl(`username`,`order_number`,`product_id`) VALUES ('$username','$orderNo','$product_id')") or die (mysqli_error($connection));
        if($quer){
            header('Location: view_product_ext_user.php?s='.urlencode("<b>Product added to cart. </b>")); 		
            exit;
        }
    }
}
                ...
                <tr>
					...
					echo "</tr></thead><tbody></tbody></table>";					
						for($l=0; $l<sizeof($pic); $l++){
							...
							echo "<img src='user_products/$registration_id/$pic[$l]' alt='Product Picture' width='250' height='250' />&nbsp;&nbsp;&nbsp;&nbsp;";
							echo "</a>";
						}
					echo"</div></div></div>";
                $i++;
