	...
	<section class="case_w3ls  py-lg-5">
		<div class="container py-5">
			<div class="text-center wthree-title pb-sm-5 pb-3">
				<h3 class="w3l-sub">Delete Member</h4>
			</div>
			<!-- case studies row -->
			<div class="row">
...
$sql=mysqli_query($connection, "SELECT * FROM registration_tbl WHERE registration_id='$sid'") or die (mysqli_error($connection));
$num=mysqli_num_rows($sql);
if(!empty($num)){
  $query=mysqli_query($connection, "DELETE FROM registration_tbl WHERE registration_id='$sid'") or die (mysqli_error($connection));
  if($query){
	unset($_SESSION['registration_id']);
    header('Location: view_member.php?s='.urlencode("<b>Member Record Successfully Deleted</b>")); 		
    exit;									        
  }
}else{
	unset($_SESSION['registration_id']);
	header('Location: view_member.php?s='.urlencode("<b>Problem deleting member record, please try again.</b>")); 		
	exit;
	}
}
